...
workdir(){
    ...
    if [ -d /root/admin/nodenest/proyectos/pokenest ]; then
        cd /root/admin/nodenest/proyectos/pokenest
        echo "Directorio cambiado a: $(pwd)" >> /root/logs/informe_nest.log
    else
        echo "ERROR: Directorio /root/admin/nodenest/proyectos/pokenest no existe" >> /root/logs/informe_nest.log
        exit 1
    fi
}
