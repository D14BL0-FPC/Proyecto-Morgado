...
load_entrypoint_postgres(){
    log "Cargando entrypoint PostgreSQL..."
    
    if [ -f /root/admin/postgres/start.sh ]; then
        bash /root/admin/postgres/start.sh || log "ADVERTENCIA: Entrypoint PostgreSQL falló, continuando..."
...
directorio_de_trabajo(){
    ...
    if cd /root/admin/nodenest/proyectos/pokenest; then
